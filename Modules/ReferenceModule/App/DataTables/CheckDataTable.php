...
class CheckDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        return (new EloquentDataTable($query))
            ...
            ->editColumn('Name in sheet', function ($model) {
                return $model->name;
            })


            ->editColumn('Code in sheet', function ($model) {
                return $model->code;
            })

            ->editColumn('Reference Name', function ($model) {
                // return $model->reference_name;
                return $model->reference_name ? $model->reference_name : 'N/A';
            })
            ->editColumn('type', function ($model) {
                // return $model->reference_name;
                return $model->type;
            })
            ->editColumn('Reference Code', function ($model) {
                return $model->code2 ? $model->code2 : 'N/A';
            })

            ->rawColumns(['name', 'code', 'reference_name', 'code2'])
            ->setRowId('id');
    }
     ...
     */
    public function query(CheckData $model): QueryBuilder
    {
...
window.LaravelDataTables['check_data_table'].columns.adjust().draw();
window.LaravelDataTables['check_data_table'].on('select deselect',function(){
var count = window.LaravelDataTables['check_data_table'].rows({ selected: true }).count();
$('.selected-item').text(count);
$('.selected-badge').text(count);
window.LaravelDataTables['check_data_table'].button('copy:name').enable(count > 0);
window.LaravelDataTables['check_data_table'].button(2).enable(count > 0);
window.LaravelDataTables['check_data_table'].button('export:name').enable(count > 0);


// Clear the set before updating
checkBoxIds.clear();
// Iterate over each selected row
window.LaravelDataTables['check_data_table'].rows({ selected: true }).every(function(rowIdx) {
...
window.LaravelDataTables['check_data_table'].rows({ selected: false }).every(function(rowIdx) {
    // Get the corresponding checkbox and check it
    var checkbox = $(this.node()).find('td:eq(0) input[type=\"checkbox\"]');
            ...
            arrOfFilterBtn.push({
                ...
                action: function (e, dt, node, config, cb) {
                    ...
                    $('#' + id).prop('checked', function (_, oldProp) {
                        if (oldProp) {
window.LaravelDataTables['check_data_table'].columns(index + 1).search("").draw();
    ...
    arrOfFilterBtn.push({
        ...
        action: function(e, dt, node, config, cb) {
            ...
            $('.filter-modal input[type="text"]').on('input', function() {
                ...
                window.LaravelDataTables['check_data_table'].columns(filterIndex).search(searchValue).draw();
            });
        }
    });
...
/*                             [
      ...
      $('.selected-item').text(window.LaravelDataTables['check_data_table'].rows({ selected: true }).count());
      $('.selected-badge').text(window.LaravelDataTables['check_data_table'].rows({ selected: true }).count());
    },lengthMenu: [
        [100,200],
        [100,200]
    ]",
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('Name in sheet')->addClass('text-center'),

            //Column::make('permissions')->addClass('text-center'),
            Column::make('Code in sheet')->addClass('text-center'),
            Column::make('Reference Name')->addClass('text-center'),
            Column::make('Reference Code')->addClass('text-center'),
