    ...
    type="button" data-bs-toggle="dropdown" data-boundary="window" aria-haspopup="true" aria-expanded="false" data-bs-reference="parent">
        <i class="fa-solid fa-ellipsis-vertical fa-xl"></i>
            ...
            <div class="dropdown-menu dropdown-menu-end py-2">';
                $html .= '<a href="#" data-bs-toggle="modal" data-bs-target="#editModal' . $model->id . '" data-id="' . $model->id . '" class="dropdown-item editReference">Edit</a>';
                $html .= '<a href="#" data-bs-toggle="modal" data-bs-target="#deleteModal' . $model->id . '" data-id="' . $model->id . '" class="dropdown-item deleteReference">Delete</a>';
...
window.LaravelDataTables['references_table'].columns.adjust().draw();
window.LaravelDataTables['references_table'].on('select deselect',function(){
var count = window.LaravelDataTables['references_table'].rows({ selected: true }).count();
$('.selected-item').text(count);
$('.selected-badge').text(count);
window.LaravelDataTables['references_table'].button('copy:name').enable(count > 0);
window.LaravelDataTables['references_table'].button(2).enable(count > 0);
window.LaravelDataTables['references_table'].button('export:name').enable(count > 0);


// Clear the set before updating
checkBoxIds.clear();
// Iterate over each selected row
window.LaravelDataTables['references_table'].rows({ selected: true }).every(function(rowIdx) {
...
window.LaravelDataTables['references_table'].rows({ selected: false }).every(function(rowIdx) {
    // Get the corresponding checkbox and check it
    var checkbox = $(this.node()).find('td:eq(0) input[type=\"checkbox\"]');
            ...
            arrOfFilterBtn.push({
                ...
                action: function (e, dt, node, config, cb) {
                    ...
                    $('#' + id).prop('checked', function (_, oldProp) {
                        if (oldProp) {
window.LaravelDataTables['references_table'].columns(index + 1).search("").draw();
    ...
    arrOfFilterBtn.push({
        ...
        action: function(e, dt, node, config, cb) {
            ...
            $('.filter-modal input[type="text"]').on('input', function() {
                ...
                window.LaravelDataTables['references_table'].columns(filterIndex).search(searchValue).draw();
            });
        }
    });
...
JS;
      ...
      $('.selected-item').text(window.LaravelDataTables['references_table'].rows({ selected: true }).count());
      $('.selected-badge').text(window.LaravelDataTables['references_table'].rows({ selected: true }).count());
    },lengthMenu: [
        [100,200],
        [100,200]
    ]",
