...
class RelatedDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        type="button" data-bs-toggle="dropdown" data-boundary="window" aria-haspopup="true" aria-expanded="false" data-bs-reference="parent">
         ...
         $html .= '<a href="#" data-bs-toggle="modal" data-bs-target="#editModal' . $model->id . '" data-id="' . $model->id . '" class="dropdown-item editRelated">Edit</a>';
         $html .= '<a href="#" data-bs-toggle="modal" data-bs-target="#deleteModal' . $model->id . '" data-id="' . $model->id . '" class="dropdown-item deleteRelated">Delete</a>';
            ...
            ->editColumn('reference_name', function ($model) {
                return $model->reference ? $model->reference->name : 'N/A';
            })
            ->editColumn('reference_code', function ($model) {
                return $model->reference ? $model->reference->code : 'N/A';
            })
     ...
     */
    public function query(Related $model): QueryBuilder
    {
    ...
    public function html(): HtmlBuilder
    {
        $initCompleteFunction = "function() {
            ...
            window.LaravelDataTables['related_table'].columns.adjust().draw();
            window.LaravelDataTables['related_table'].on('select deselect',function(){
            var count = window.LaravelDataTables['related_table'].rows({ selected: true }).count();
            $('.selected-item').text(count);
            $('.selected-badge').text(count);
            window.LaravelDataTables['related_table'].button('copy:name').enable(count > 0);
            window.LaravelDataTables['related_table'].button(2).enable(count > 0);
            window.LaravelDataTables['related_table'].button('export:name').enable(count > 0);


            // Clear the set before updating
            checkBoxIds.clear();
            // Iterate over each selected row
            window.LaravelDataTables['related_table'].rows({ selected: true }).every(function(rowIdx) {
            ...
            window.LaravelDataTables['related_table'].rows({ selected: false }).every(function(rowIdx) {
                // Get the corresponding checkbox and check it
                var checkbox = $(this.node()).find('td:eq(0) input[type=\"checkbox\"]');
            ...
            localStorage.setItem('related_checkBoxIdsArray', JSON.stringify(checkBoxIdsArray))
                        ...
                        arrOfFilterBtn.push({
                            ...
                            action: function (e, dt, node, config, cb) {
                                ...
                                $('#' + id).prop('checked', function (_, oldProp) {
                                    if (oldProp) {
            window.LaravelDataTables['related_table'].columns(index + 1).search("").draw();
                ...
                arrOfFilterBtn.push({
                    ...
                    action: function(e, dt, node, config, cb) {
                        ...
                        $('.filter-modal input[type="text"]').on('input', function() {
                            ...
                            window.LaravelDataTables['related_table'].columns(filterIndex).search(searchValue).draw();
                        });
                    }
                });
      ...
      $('.selected-item').text(window.LaravelDataTables['related_table'].rows({ selected: true }).count());
      $('.selected-badge').text(window.LaravelDataTables['related_table'].rows({ selected: true }).count());
    },lengthMenu: [
        [100,200],
        [100,200]
    ]",
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('reference_name')->addClass('text-center'),
            Column::make('reference_code')->addClass('text-center'),
